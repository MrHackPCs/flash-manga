...
export class FlashManga extends Source {
    ...
    override async getChapters(mangaId: string): Promise<Chapter[]> {
        ...
        let chapters = FlashMangaParser.parseChapterList($, mangaId)
        ...
        if (chapters.length === 0) {
            try {
                ...
                chapters = FlashMangaParser.parseChapterList($, mangaId)
            } catch (err) {
    ...
    override async getChapterDetails(mangaId: string, chapterId: string): Promise<ChapterDetails> {
        ...
        try {
            ...
            // Fallback for flat chapter URL structure if applicable
    ...
    override async getViewMoreItems(homepageSectionId: string, metadata: any): Promise<PagedResults> {
        ...
        switch (homepageSectionId) {
            case 'popular':
                url = page > 1
                    ? `${FLASH_MANGA_DOMAIN}/manga/page/${page}/?m_orderby=views`
                    : `${FLASH_MANGA_DOMAIN}/manga/?m_orderby=views`
                break
            case 'latest':
                url = page > 1
                    ? `${FLASH_MANGA_DOMAIN}/manga/page/${page}/?m_orderby=latest`
                    : `${FLASH_MANGA_DOMAIN}/manga/?m_orderby=latest`
                break
            case 'all':
            default:
                url = page > 1
                    ? `${FLASH_MANGA_DOMAIN}/manga/page/${page}/?m_orderby=alphabet`
                    : `${FLASH_MANGA_DOMAIN}/manga/?m_orderby=alphabet`
                break
        }
    ...
    override async getSearchResults(query: SearchRequest, metadata: any): Promise<PagedResults> {
        ...
        if (query.title) {
            url = page > 1
                ? `${FLASH_MANGA_DOMAIN}/page/${page}/?s=${encodeURIComponent(query.title)}&post_type=wp-manga`
                : `${FLASH_MANGA_DOMAIN}/?s=${encodeURIComponent(query.title)}&post_type=wp-manga`
        } else {
            url = page > 1
                ? `${FLASH_MANGA_DOMAIN}/manga/page/${page}/?m_orderby=latest`
                : `${FLASH_MANGA_DOMAIN}/manga/?m_orderby=latest`
