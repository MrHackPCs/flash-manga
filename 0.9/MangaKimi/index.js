...
var _Sources = (() => {
  ...
  // src/MangaKimi/MangaKimi.ts
  var MangaKimi_exports = {};
  __export(MangaKimi_exports, {
    MangaKimi: () => MangaKimi,
    MangaKimiInfo: () => MangaKimiInfo
  });
  ...
  // src/MangaKimi/MangaKimiHelper.ts
  var MANGA_KIMI_DOMAIN = "https://www.mangakimi.com";
  ...
  function parseChapterNumber(chapterStr) {
  ...
  function parseThaiDate(dateStr) {
    ...
    if (cleanDate.includes("\u0E19\u0E32\u0E17\u0E35") || cleanDate.includes("minute")) {
      ...
      return new Date(now.getTime() - mins * 60 * 1e3);
    }
    ...
    if (parts.length >= 3) {
      ...
      let month = 0;
      for (const [tMonth, index2] of Object.entries(thaiMonths)) {
  ...
  // src/MangaKimi/MangaKimiParser.ts
  var MangaKimiParser = class {
    /**
     * Parses manga tiles from standard MangaReader list item grids (.bs .bsx)
     */
    static parseMangaTiles($2) {
      const tiles = [];
      $2(".bs .bsx").each((_, element) => {
        const linkTag = $2("a", element).first();
        const href = linkTag.attr("href") || "";
        const title = linkTag.attr("title")?.trim() || $2(".tt", element).text().trim();
        ...
        const subtitle = $2(".adds .epxs, .adds .epx, .epx, .epxs", element).first().text().trim() || void 0;
        tiles.push(
          App.createMangaTile({
            id,
    ...
    static parseMangaDetails($2, mangaId) {
      const title = $2("h1.entry-title").text().trim() || $2(".info-right h1").text().trim();
      const imgTag = $2(".info-left .thumb img, .thumb img").first();
      const image = imgTag.attr("data-src") || imgTag.attr("src") || $2('meta[property="og:image"]').attr("content") || "";
      const description = $2('.entry-content-single[itemprop="description"] p, .entry-content-single, .desc').text().trim();
      let status = "Ongoing";
    ...
    static parseChapterList($2, mangaId) {
      ...
      $2("#chapterlist ul li, .bxcl ul li, .eplister ul li").each((_, element) => {
        ...
        if (!chapterId) return;
        const name = $2(".chapternum", element).text().trim() || $2("a", element).text().trim();
  ...
  var MangaKimi = class extends import_types2.Source {
    constructor() {
      super(...arguments);
    ...
    async getMangaDetails(mangaId) {
      ...
      return MangaKimiParser.parseMangaDetails($2, mangaId);
    }
    /**
    ...
    async getChapters(mangaId) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
      return MangaKimiParser.parseChapterList($2, mangaId);
    }
    /**
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const $2 = load(html3);
      return MangaKimiParser.parseChapterDetails(html3, mangaId, chapterId, $2);
    }
    /**
    ...
    async getHomePageSections(sectionCallback) {
      ...
      try {
        ...
        const $2 = load(response.data);
        try {
          const popularTiles = MangaKimiParser.parseMangaTiles($2(".top10content, .hothome"));
        ...
        try {
          const latestTiles = MangaKimiParser.parseMangaTiles($2(".postbody .bixbox:not(.hothome)"));
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
      const items = MangaKimiParser.parseMangaTiles($2);
      return App.createPagedResults({
        results: items,
        metadata: MangaKimiParser.hasNextPage($2) ? { page: page + 1 } : void 0
      });
    }
    /**
  ...
  return __toCommonJS(MangaKimi_exports);
})();

this.Sources = _Sources;
if (typeof exports === 'object' && typeof module !== 'undefined') {
    module.exports.Sources = this.Sources;
    module.exports.MangaKimi = _Sources.MangaKimi;
    module.exports.MangaKimiInfo = _Sources.MangaKimiInfo;
}
if (typeof globalThis !== 'undefined') {
    globalThis.Sources = _Sources;
    globalThis.MangaKimi = _Sources.MangaKimi;
    globalThis.MangaKimiInfo = _Sources.MangaKimiInfo;
}
